<template>
  ...
  <div
    class="fixed z-50 hidden md:flex backdrop-blur-md bg-white/70 dark:bg-[#0F172A]/80 items-center h-16 px-4 md:px-8 border-b border-slate-200/50 dark:border-white/5 transition-all duration-500"
  ...
  <!-- ASIDE: Sidebar Azul BM Labs -->
  <aside
    :class="[
      'w-60 fixed top-0 left-0 bottom-0 h-screen flex flex-col transition-all duration-500 cubic-bezier-sidebar',
  ...
  >
    ...
    <div class="h-16 flex items-center justify-end px-3 overflow-hidden shrink-0 border-b border-white/10">
      <Transition name="fade">
        <div :class="railMode ? 'flex-1 flex items-center justify-center px-2' : 'flex-1 flex items-center px-2 gap-3'">
          <AppLogo :size="railMode ? 40 : 56" />
          
          <div v-if="!railMode" class="flex items-center">
            <span class="text-xl font-black tracking-tighter text-white">{{ BRAND_CONFIG.name.first }}</span>
            <span class="text-xl font-black tracking-tighter text-blue-300 ml-1">{{ BRAND_CONFIG.name.last }}</span>
          </div>
        </div>
      </Transition>
      ...
      <button
        type="button"
        class="hidden md:flex w-10 h-10 items-center justify-center rounded-xl bg-white/10 text-white hover:bg-white/20 transition-all active:scale-90 shrink-0"
        @click.stop="openNav"
    ...
    <Teleport to="body">
      <template v-if="flyoutGroup && railMode">
        ...
        <div
          class="fixed z-[59] w-56 rounded-2xl border border-white/10 shadow-2xl backdrop-blur-3xl py-3 animate-in fade-in slide-in-from-left-4 duration-300"
          :style="{ top: `${flyoutTop}px`, left: '72px', backgroundColor: 'var(--sidebar-bg)' }"
        >
          <p class="px-5 pb-2 mb-2 border-b border-white/10 text-[10px] font-black uppercase tracking-widest text-white/70">
            {{ flyoutGroup.label }}
          </p>
          <div class="px-2 space-y-1">
            <button
              ...
              :class="isActive(child) ? 'bg-white text-[var(--sidebar-bg)]' : 'text-white/70 hover:bg-white/10 hover:text-white'"
              @click="navigate(child.path)"
            >
              {{ child.label }}
            </button>
...
import { LogOut, Search, ChevronLeft, ChevronRight, Sun, Moon, Menu } from 'lucide-vue-next'
